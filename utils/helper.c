...
{
    if (mem->ptr >= mem->vmem_start - mem->content) 
    {
        return -1;
    } //End of memory
    ...
    switch (op) {
        ...
        case OP_STM: {
            ...
            uint16_t ptr = reg_val(state->sto[2], reg);
        ...
        case OP_AND: {
            ...
            uint16_t val_rhs = reg_val(state->sto[2], reg);
        ...
        case OP_JNZ: 
        {
            if (reg_val(state->sto[1],reg) != 0) mem->ptr = reg->PC;
            reg->PC = mem->ptr;
            return 0;
        }
        case OP_SPC: 
        {
            mem->ptr = reg_val(state->sto[1], reg);
            return 0;
        }
        case OP_STL: 
        {
